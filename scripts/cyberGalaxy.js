...
function generateEnemySpawnPoint() {
	let enemySpawnCoordinateX;
	let enemySpawnCoordinateY;
	const enemySpawnX = [
		-4,
		-2,
		0,
	...
	enemySpawnCoordinateX = enemySpawnX[matrixRandomIndexX];
	enemySpawnCoordinateY = enemySpawnY[matrixRandomIndexY];   
	return {enemySpawnCoordinateX, enemySpawnCoordinateY};
}
...
function addEnemy(){
	const {enemySpawnCoordinateX, enemySpawnCoordinateY } = generateEnemySpawnPoint();
	const geometry = new THREE.BoxGeometry( 1, 1, 1 );
	const material = new THREE.MeshBasicMaterial( {color: 0x00ff00} );
	const enemy = new THREE.Mesh( geometry, material );

	enemy.position.x = enemySpawnCoordinateX;
	enemy.position.y = enemySpawnCoordinateY;
	enemy.position.z = -60;
	enemy.health = 100;
